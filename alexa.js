...
module.exports = alexa = async (alexa, m, chatUpdate, store) => {
    try {
        ...
        const botNumber = await alexa.decodeJid(alexa.user.id)
        const isCreator = [botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        const itsMe = m.sender == botNumber ? true : false
		 const botName = process.env.NAME || "Alexa"
		const ter = args.join(' ') 
        ...
        const groupMetadata = m.isGroup ? await alexa.groupMetadata(m.chat).catch(e => {}) : ''
...
if (icmd) {	
    ...
    if (hasLeveledUp) {
        ...
        await alexa.sendMessage(m.chat, { image: { url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSR5kFSuoFniw90CNXW8z1FkDma4WF6fJyL3Q&usqp=CAU" }, caption: `* LEVEL UP! *\n\n*ðŸ“ƒï¸Name*: ${pushname}\n*ðŸ€Exp*: ${user.xp} / ${Levels.xpFor(user.level + 1)}\n*ðŸŽLevel*: ${user.level} ðŸ†™ \n*ðŸ”®ï¸Role*: *${role}*\n\nCongrats!! ðŸŽ‰ðŸŽ‰`}, { quoted: m });
    }
}
		try {
...
if (budy.includes(`//chat.whatsapp.com/${response}`)) return
        ...
        await alexa.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
    }
    }
		}
		} catch (err) {
	 ...
	 /////////////////////////////////////
        ...
        if (m.message) {
           // alexa.sendReadReceipt(m.chat, m.sender, [m.key.id])
    ...
    let Iban = await user.findOne({ id: m.sender})
		if(icmd && Iban.ban == 'true') return m.reply(`You are banned from using commands âŒ`)
...
case '':
    ...
    const dbut = [
{buttonId: `${prefix}help`, buttonText: {displayText: 'ðŸš€Commands'}, type: 1},
{buttonId: `${prefix}info`, buttonText: {displayText: 'ðŸŽBot status'}, type: 1}
]
let buttonMessaged = {
 ...
 await alexa.sendMessage(m.chat,buttonMessaged,{quoted:m})
}
 break
...
break
 ...
 case 'info':{
	    ...
	    const dbut = [
{buttonId: `${prefix}help`, buttonText: {displayText: 'ðŸCommandsðŸ'}, type: 1}
]
    const uptime = process.uptime()
	let tr = await user.countDocuments()
	                let getGroups = await alexa.groupFetchAllParticipating()
                let groups = Object.entries(getGroups).slice(0).map(entry => entry[1])
                let anu = groups.map(v => v.id)
	let ur = anu.length
	let b = await user.countDocuments({ ban : "true" })
let ter = `ðŸ“š *Commands*: 55\nðŸ‘¥ *Users*: ${tr}\nðŸ”° *Groups*: ${ur}\nðŸš« *Ban_Users*: ${b}\nâ›© *Moderators*: ${owner.length}`
console.log(ter)
   let buttonMessaged = {
        text: ter,
        footer: 'Â©ALEXA',
...
break
		   ...
		   case "help":
                   case 'menu':
                   const hlp=`
*(â¤ï¸Ï‰â¤ï¸)Kon'nichiwa ðŸ‘‹ðŸ» watashiwa ${pushname} Darling I'm Alexa Queen*

ðŸ“Note: if Bot did not responding then it's offline or owner internet issue so please wait until Bot comes online.!

ðŸŽMy Prefix is:- ${prefix}

â­ ð‚ðŽðŒðŒð€ððƒ ð‹ðˆð’ð“ â­

â”â”â”â”ã€ŽðŸDevðŸã€â”â”â”â”
â update, broadcast, ban, unban, chatid, join, session

â”â”â”ã€ŽðŸ€GeneralðŸ€ã€â”â”â”
â help, delete, info, rank, profile

â”â”â”â”ã€ŽðŸ’«FunðŸ’«ã€â”â”â”â”
â ship, advice, quote, leaderboard, react, fact, bot

â”â”â”ã€ŽðŸŒ²MediaðŸŒ²ã€â”â”â”
â yta, ytv, play, gify, yts, lyrics

â”â”â”ã€ŽðŸ”°ModerationðŸ”°ã€â”â”â”
â demote, remove, ping, promote, deact, act, invite, group, seticon, setdesc

â”â”â”ã€ŽðŸ®WeebðŸ®ã€â”â”â”
...
â google, image, meme, subreddit, tourl, img, iguser, gif, sticker, take, pinterest, carbon
 ...
 ðŸŽ ð™‹ð™¤ð™¬ð™šð™§ð™šð™™ ð™—ð™® ð—”ð—¹ð—²ð˜…ð—® ð—¤ð˜‚ð—²ð—²ð—» ðŸŽ`			     
const hhbut = [
{buttonId: `{prefix}info`, buttonText: {displayText: 'ðŸ“¥Info'}, type: 1},
{buttonId: `prefix}profile`, buttonText: {displayText: 'â­Profile'}, type: 1}
]
let hbutto = {
        file: alexa.sendMessage(m.chat,{video:fs.readFileSync('./trash/help.mp4'),gifPlayback:true,caption:hlp},{quoted:m}),
        caption: hlp,
        footer: 'Â©Alexa 2022',
        buttons: hhbut,
        headerType: 4
    }
alexa.sendMessage(m.chat,hbutto,{quoted:m})

			   break
		   case 'session': {
			   if (!isCreator) return m.reply("ðŸ“The user of this command must be the owner of the bot")
  ...
  const mems_id = new Array()
      const lb = await Levels.fetchLeaderboard("bot", 10);
					 let lbtext = "*â”â”â”â”ã€ŽðŸ’«LeaderBoardðŸ’«ã€â”â”â”â”*\n\n"
...
try {
     ...
     const bufferr =  await Carbon.generateCarbon(carbon) 
       alexa.sendMessage(m.chat, {image: bufferr}, {quoted: m})
...
case 'gify': 
 ...
 alexa.sendMessage(m.chat, { video: { url: gi.results?.[Math.floor(Math.random() * gi.results.length)]?.media[0]?.mp4?.url }, caption: "Here you go",gifPlayback: true }, { quoted: m })
		} catch (err) {
...
const userq = await Levels.fetch(m.sender, "bot");
		...
		let disc = m.sender.substring(3, 7)
			...
			textr += `*ðŸ€Exp*: ${userq.xp} / ${Levels.xpFor(userq.level + 1)}\n*ðŸŽLevel*: ${userq.level}\n*ðŸ”®ï¸Role*: ${role}`
			
			try {
                    ppuser = await alexa.profilePictureUrl(m.sender, 'image')
                } catch {
...
sendMessage(m.chat,{image:data,caption:textr},{quoted:m})
  })
...
case  'sticker': case 's': case 'stickergif': case 'sgif':
   {
	 if (!quoted) return m.reply(`âŒ Could not find any Image/Video in context`)
		 if (q) {
	anu = args.join(' ').split('|')
...
alexa.sendMessage(m.chat, {sticker: buffer}, {quoted: m})
    } else if (/video/.test(mime)) {
        if ((quoted.msg || quoted).seconds > 20) return m.reply('ðŸ• Cannot fetch videos longer than *21 Seconds*')
        let media = await quoted.download()
    let sticker = new Sticker(media, {
        pack: pack, // The pack name
        author: author, // The author name
    ...
    alexa.sendMessage(m.chat, {sticker: stikk}, {quoted: m})
    } else {
        m.reply("âŒ Could not find any Image/Video in context")
        }
...
const userw = await Levels.fetch(m.sender, "bot");
		...
		let bio = ''
try {
bio = (await alexa.fetchStatus(m.sender)).status
} catch (error) {
	console.log(error)
bio = 'None'
}
let disec = m.sender.substring(3, 7)
			...
			tex += `*ðŸ•¹XP*: ${userw.xp} / ${Levels.xpFor(userw.level + 1)}\n\n*â¤Level*: ${userw.level}\n\n*ðŸ® Role*: ${role}\n`
			...
			try {
				...
				alexa.sendMessage(m.chat, { image: { url: ppuser }, caption: tex }, { quoted: m })
break
  ...
  case 'img': case 'togif': case 'toimg': case 'gif': {
	  if(m.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated !== true) {
		                  if (!quoted) return m.reply(`âŒ Could not find any sticker in context`)
                if (!/webp/.test(mime)) return m.reply(`âŒ Couldn't find any sticker in context`)
                m.reply("wait your request is under process")
                let media = await alexa.downloadAndSaveMediaMessage(quoted)
                let ran = await getRandom('.png')
                exec(`ffmpeg -i ${media} ${ran}`, (err) => {
                    fs.unlinkSync(media)
                    if (err) m.reply(err)
                    let buffer = fs.readFileSync(ran)
                    alexa.sendMessage(m.chat, { image: buffer,caption:'Â©ALEXA-QUEEN-2022' }, { quoted: m })
                    fs.unlinkSync(ran)
                })
        ...
        let { webp2mp4File } = require('./lib/uploader')
                ...
                await alexa.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Â©ALEXA-QUEEN-2022' }, gifPlayback: true }, { quoted: m })
...
if (!isAdmins) return m.reply("âŒ This is an Admin only Command")
if (!isBotAdmins) return m.reply("âŒ Cannot execute without being admin")
	try {
	  ...
	  if (!users) return m.reply("âŒ Couldn't find any userID in context")
        await alexa.groupParticipantsUpdate(m.chat, [users], 'remove')
 alexa.sendMessage(m.chat,{text:`Kicked @${users.split("@")[0]} successfuly `,contextInfo: { mentionedJid: [users] }})
 	} catch {
	return m.reply("âŒ Please tag the user you want to kick")
	}
    }
    break
    case 'promote': {
...
if (!isAdmins) return m.reply("âŒ This is an Admin only Command")
if (!isBotAdmins) return m.reply("âŒ Cannot execute without being admin")
                 if (!q) m.reply(`âŒ No query provided!`)
                await alexa.groupUpdateDescription(m.chat, text)
...
if (!isAdmins) return m.reply("âŒ This is an Admin only Command")
if (!isBotAdmins) return m.reply("âŒ Cannot execute without being admin")
                if (!/image/.test(mime)) m.reply("âŒ Could not find any Image in context")
                if (/webp/.test(mime)) m.reply("âŒ Could not find any Image in context")
                let media = await alexa.downloadAndSaveMediaMessage(quoted)
                await alexa.updateProfilePicture(m.chat, { url: media }).catch((err) => fs.unlinkSync(media))
                m.reply('ðŸ’¡ Group icone has been changed.')
                }
                break 
case 'tagall':
case 'ping':
...
if (!isAdmins) return m.reply("âŒ This is an Admin only Command")
if (!isBotAdmins) return m.reply("âŒ Cannot execute without being admin")
if(q) { var Text =`ðŸ“Œ *Message - ${q}*\n*ðŸ Group name - ${groupName}*` } else {  var Text = `*${groupName}*`}

let menText = `${Text}\n*ðŸ’« ping by - ${pushname}*\n*ðŸ•› time - ${time}*\n\n`
for (let memNum of participants) {
    
    if( groupAdmins.includes(memNum.id) === true ) { var emo = 'ðŸ‘‘'} else { var emo = 'â„ï¸'} 
    menText += `${emo} *@${memNum.id.split('@')[0]}*\n`
    //members_id.push(memNum.jid)
}
alexa.sendMessage(m.chat,{text:menText,mentions: participants.map(a => a.id)},{quoted:m})
break
...
if (!isBotAdmins) return m.reply("âŒ Cannot execute without being admin")
             if (args[0] === 'open'){
                await alexa.groupSettingUpdate(m.chat, 'not_announcement').then((res) => m.reply(`*Group open*`)).catch((err) => m.reply(jsonformat(err)))
             } else if (args[0] === 'close'){
                await alexa.groupSettingUpdate(m.chat, 'announcement').then((res) => m.reply(`*Group closed*`)).catch((err) => m.reply(jsonformat(err)))
             } else {
					const sections = [
    {
...
const listMessage = {
  ...
  sections
                    ...
                    await alexa.sendMessage(m.chat, listMessage)
...
case 'linkgroup': case 'invitelink': {
 ...
 break
  case 'delete': case 'del': {
                if (!m.quoted) m.reply("ðŸ“Ž Tag the massage you want to delete")
                let { chat, fromMe, id, isBaileys } = m.quoted
                if (!isBaileys) m.reply('âŒ I can not delete massage from another userID except mine.')
                alexa.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: true, id: m.quoted.id, participant: m.quoted.sender } })
            }
            break
  case 'enable':
  case 'register':
 case 'act':
 if (!ter) return m.reply(`âŒ No option provided!`)
...
return m.reply('ðŸ’® Successfully Enabled *Mod*')
		 } else {
		 if(gc.mod == "true") return m.reply("ðŸ›¡ *Mod* is already enabled")
...
return m.reply("ðŸ’® Successfully Enabled *Events*")
		 } else {
		 if(gc.events == "true") return m.reply("ðŸ—ž *Events* is already enabled")
...
return m.reply("ðŸŽ Successfully Enabled *Invite*")
		 } else {
		 if(gc.invite == "true") return m.reply("ðŸ–Œï¸ *Invite* is already enabled")
									await group.updateOne({ id: m.chat }, {invite: "true"})
								console.log(gc.invite)
							return m.reply("ðŸŒ Successfully Enabled *Invite*")
...
return m.reply("ðŸ’« Successfully Enabled *NSFW*")
		 } else {
		 if(gc.nsfw == "true") return m.reply("ðŸ’« *NSFW* is already enabled")
...
group.findOne({ id : m.chat }).then(async(usr) => {
         if (!usr) {
return m.reply("ðŸ›¡ *Mod* is already disabled")
		 } else {
		 if(usr.mod !== "true") return m.reply("ðŸ›¡ *Mod* is already disabled")
									await group.updateOne({ id: m.chat }, {mod: "false"})
return m.reply("ðŸ§© Successfully Disabled *Mod*");
						}							
				})
 }else if (args[0] == "events") { 
group.findOne({ id : m.chat }).then(async(usr) => {
         if (!usr) {
 return m.reply("ðŸ—ž *Events* is already disabled")
		 } else {
		 if(usr.events !== "true") return m.reply("ðŸ—ž *Events* is already disabled")
									await group.updateOne({ id: m.chat }, {events: "false"})
return m.reply("ðŸ§© Successfully Disabled *Events*"); 
						}							
				})
 } else if (args[0] == "invite") { 
group.findOne({ id : m.chat }).then(async(usr) => {
         if (!usr) {
 return m.reply("ðŸŽ *Invite* is already disabled")
		 } else {
		 if(usr.invite !== "true") return m.reply("ðŸŽ *Invite* is already disabled")
									await group.updateOne({ id: m.chat }, {invite: "false"})
return m.reply("ðŸ§© Successfully Disabled *Invite*"); 
						}							
				})
 } else if (args[0] == "nsfw") { 
group.findOne({ id : m.chat }).then(async(usr) => {
         if (!usr) {
 return m.reply("ðŸ§§ *NSFW* is already disabled")
		 } else {
		 if(usr.nsfw !== "true") return m.reply("ðŸ§§ *NSFW* is already disabled")
									await group.updateOne({ id: m.chat }, {nsfw: "false"})
return m.reply("ðŸ§© Successfully Disabled *NSFW*"); 
						}							
				})
 }
 else {
...
return m.reply("âŒ Couldn't find any userID in context")
		 }
...
m.reply(`ðŸ•£ Downloading ${ter}`)
    let search = await yts(ter)
    let quality = args[1] ? args[1] : '128kbps'
    let media = await yta(search.all[0].url, quality)
	alexa.sendMessage(m.chat, { audio: { url: media.dl_link }, contextInfo: {
                    externalAdReply: {
...
case 'ytmp3': case 'ytaudio': case 'yta': {
    const getRandom = (ext) => {
        ...
        if (!urlYt.startsWith("http")) {
          reply(`âŒ Give youtube link!`);
          return;
        }
        let infoYt = await ytdl.getInfo(urlYt);
        //30 MIN
        if (infoYt.videoDetails.lengthSeconds >= 1800) {
          reply(`âŒ Video too big!`);
          return;
        }
        ...
        if (fileSizeInMegabytes <= 40) {
          ...
          reply(`âŒ File size bigger than 40mb.`);
        }
...
case 'ytmp4': case 'ytvideo': case 'ytv' :
    const getRandom = (ext) => {
        ...
        if (args.length === 0) {
          m.reply(`âŒ URL is empty! \nSend ${prefix}ytv url`);
          return;
        }
        let urlYt = args[0];
        if (!urlYt.startsWith("http")) {
          m.reply(`âŒ Give youtube link!`);
          return;
        }
        let infoYt = await ytdl.getInfo(urlYt);
        //30 MIN
        if (infoYt.videoDetails.lengthSeconds >= 1800) {
          m.reply(`âŒ Video file too big!`);
          return;
        }
        let titleYt = infoYt.videoDetails.title;
        ...
        if (fileSizeInMegabytes <= 100) {
          ...
          m.reply(`âŒ File size bigger than 40mb.`);
        }
      
        fs.unlinkSync(`./${randomName}`);
break
case 'yts': case 'ytsearch': {
    if (!ter) return m.reply(`âŒNo query provided!`)
    let yts = require("yt-search")
    let search = await yts(ter)
    let teks = 'YouTube Search\n\n Result From '+ ter +'\n\n'
    let no = 1
    for (let i of search.all) {
        teks += `*#${no++}*\nðŸœï¸ *Title*: ${i.title}\nðŸŒ¸ *Duration*: ${i.timestamp}\nðŸŒ *Url*: ${i.url}\n`
    }
    alexa.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },jpegThumbnail:fs.readFileSync('./trash/yts.jpg'),  caption: teks, }, { quoted: m, })
}
break
case 'sr':
case 'subraddit': {
	...
	if (res.data.nsfw&& hh == 'false') return m.reply("âŒ *nsfw* is not active in this group")
		if (!res.data.url) return m.reply("Subraddit dosen't exsist") 
				alexa.sendMessage(m.chat,{image:{url: res.data.url},caption:`ðŸ–Œï¸ *Title:* ${res.data.title}\n*ðŸ‘¨â€ðŸŽ¨ Author:* ${res.data.author}\n*ðŸŽ Subreddit:* ${res.data.subreddit}\nðŸŒ *Post:* ${res.data.postLink}`},{quoted:m,})
...
if (spankd.data.nsfw&& sk == 'false') return m.reply("âŒ *nsfw* is not active in this group")

await alexa.sendMessage(m.chat,{video: spgif, gifPlayback:true},{ quoted:m }).catch(err => {
...
case 'meme':{
	...
	alexa.sendMessage(m.chat,{image:{url:url},caption:`*${title}*`},{quoted:m,})
}
break
case 'add': {
    ...
    await alexa.groupParticipantsUpdate(m.chat, [users], 'add')
    alexa.sendMessage(m.chat,{text:`Added @${users.split("@")[0]} successfuly `,contextInfo: { mentionedJid: [users] }})
}
break
case 'join': {
                if (!isCreator) return m.reply("ðŸ“The user of this command must be the owner of the bot")
                if (!ter) return m.reply("ðŸ” Please provide the group link")
                if (!isUrl(ter) && !ter.includes('whatsapp.com')) return m.reply("ðŸ” Please provide the group link")
                m.reply("wait your request is under process")
                let result = args[0].split('https://chat.whatsapp.com/')[1]
                await alexa.groupAcceptInvite(result).then((res) => m.reply(jsonformat(res))).catch((err) =>m.reply(`Invalid ðŸ“˜ *URL: ${q}*`))
            }
            break
            case 'leave': {
                if (!isCreator) return m.reply("ðŸ“The user of this command must be the owner of the bot")
                await alexa.groupLeave(m.chat).then((res) => m.reply(jsonformat(res))).catch((err) => m.reply("Error"))
            }
            break
...
if (!ter) return m.reply(`âŒ No query provided!`)
 ...
 await alexa.sendMessage(m.chat, reactionMessage)
  alexa.sendMessage(m.chat, { text:lyrics, contextInfo: {
    externalAdReply: {
        title:'Lyrics-',
...
case 'gimage':case 'image': {
        ...
        alexa.sendMessage(m.chat, { image: { url: images }, caption: "Here you go" }, { quoted: m })
        })
        }
        break
 case 'google': {
                ...
                teks += `*ðŸƒDescription* : ${g.snippet}\n`
                teks += `*ðŸŒLink* : ${g.link}\n\n`
                } 
                m.reply(teks)
                })
                }
...
break
			       case 'update': {
					   if (!isCreator) return m.reply("ðŸ“The user of this command must be the owner of the bot")
      stdout = execSync('git remote set-url origin https://github.com/Diegoson/ALEXA-QUEEN-V3.git && git pull')
...
jj = `\t\t\t\t\t*ShipCent : ${ll}%* \n\tAmazing! You two will be a good coupleðŸ’–`
rate = "Amazing"
} else if ( ll == 100 ){
jj = `\t\t\t\t\t*ShipCent : ${ll}%* \n\tYou two are fated to be togetherðŸ’™`
rate = "Fated to be together"
		...
		try {
				...
				if(users == 'none'){
							try {
                    ppuser2 = await alexa.profilePictureUrl(m.sender, 'image')
                } catch {
                    ppuser2 = 'https://www.linkpicture.com/q/IMG-20220118-WA0387.png'
                    }
				  } else {
												try {
                    ppuser2 = await alexa.profilePictureUrl(users, 'image')
                } catch {
                    ppuser2 = 'https://www.linkpicture.com/q/IMG-20220118-WA0387.png'
                    }
				}
	...
	alexa.sendMessage(m.chat,{image:ship,caption:caption,mentions:ment},{quoted:m})
		}
...
const reply = `
ðŸ“ *Content:* ${quoo.data.quote.body}
*âœï¸ Author:* ${quoo.data.quote.author}
              `
alexa.sendMessage(m.chat, { text:reply },  {quoted:m})
} catch (err) {
console.log(err)
return m.reply (`*âŒ Something went wrong.*`)
    }
break 
case 'fact':{
				 await axios
            ...
            .then((response) => {
                ...
                const tet = `ðŸ“› *Fact:* ${response.data.fact}`
                m.reply(tet)
            })
            .catch((err) => {
        ...
        await axios
            ...
            .then((response) => {
                // console.log(response);
                const tet = `*Advice for youðŸ”–:* ${response.data.slip.advice}`
                m.reply(tet)
            })
            .catch((err) => {
        ...
        .then((response) => {
                // console.log(response);
                const txt = `ðŸŽ *Bot*:  ${response.data.success}`
                m.reply(txt);
            }).catch(err => {
...
console.log(recp)
		...
		alexa.sendMessage(m.chat,{video: sgif, gifPlayback:true,mentions:ment,caption:recp},{quoted:m})
    } catch (error) {
        console.log(error);
    }
}
...
break
 ...
 case 'couplepp': case 'ppcouple': {
                ...
                alexa.sendMessage(m.chat, { image: { url: random.male }, caption: `Male` }, { quoted: m })
...
case 'neko':{
 ...
 alexa.sendMessage(m.chat, { image: { url: waifud.data.url }, caption: "Here you go" }, { quoted: m })
                ...
                break                                
case 'anime':
if (!q) return m.reply(`âŒ No query provided!`)
const { data: jap } = await axios.get(`https://api.jikan.moe/v3/search/anime?q=${q}`)
if(!jap.results[0].title) return m.reply(`âŒ Couldn't find any results on the term *${q}*`)
const { data } = (await axios.get(`https://api.jikan.moe/v4/anime?q=${jap.results[0].title}`)).data;
			let data3 =
				...
				}/${data[0].title_japanese}\n*ðŸ”–Trailer:* ${data[0].trailer.url}\n` +
				`*ðŸ”MAL_ID:* ${data[0].mal_id}\n*âœ´ï¸Type:* ${data[0].type}\n*ðŸŽ¬Episode(s):* ${data[0].episodes}\n*ðŸ“¢Airing:* ${data[0].status}\n*ðŸ””Date:* ${data[0].aired.string}\n` +
				`*ðŸ”±Rating:* ${data[0].rating}\n*âšœï¸Duration:* ${data[0].duration}\n*â™¨ï¸Score:* ${
					data[0].score
				}\n*ðŸ“¦Studio(s):* ${data[0].studios.map((val) => `${val.name}`).join(", ")}\n` +
				`*ðŸŽžï¸Genre(s):* ${data[0].genres.map((val) => `${val.name}`).join(", ")}\n*ðŸ“šSynopsis:* ${
					data[0].synopsis
				}`;
alexa.sendMessage(m.chat,{image:{url:data[0].images.jpg.image_url},caption:data3},{quoted:m})   
break
case 'chara':
case 'charecter':{
	if (!q) return m.reply(`âŒ No query provided!`)
		...
		const chara = await client.character(q).catch((err) => {
			return m.reply(`âŒ Couldn't find any results on the term *${q}*`)
		});
		let texty = "";
		texty += `ðŸ’™ *Name: ${chara.data.characters.results[0].name.full}*\n`;
		texty += `ðŸ’› *Source: ${chara.data.characters.results[0].media.edges[0].node.title.userPreferred}*\n\n`;
		texty += `ðŸŒ *URL: ${chara.data.characters.results[0].siteUrl}*\n\n`;
		texty += `â¤ *Description:* ${chara.data.characters.results[0].description}\n`;
			...
			let data2 =
				`*ðŸ“•Name:* ${char[0].name}\n*âšœï¸About:* ${
					char[0].about
				}\n*ðŸ”MAL_ID:* ${char[0].mal_id}\nðŸŒ *URL:*  ${chara.data.characters.results[0].siteUrl}\n` +
				`\n*ðŸ”–Appears:*${anime
					.map((val) => `\n*ðŸ”®Role:* ${val.role}\n*ðŸŽ¬Title:* ${val.anime.title}`)
					.join("\n")}\nâ¤ *Description:* ${chara.data.characters.results[0].description}\n` +
				`\n*ðŸ‘¥VA:*${voice
					.map((val) => `\n*ðŸŒLanguage:* ${val.language}\n*ðŸ‘¤Person:* ${val.person.name}`)
					.join("\n")}`;
alexa.sendMessage(m.chat,{image:{url:char[0].images.jpg.image_url},caption:data2},{quoted:m})  
}
	...
	const client = new Character();
		...
		texty += `*ðŸ’›Source:* ${chara.data.characters.results[0].media.edges[0].node.title.userPreferred}\n\n`;
			...
			let data2 =
				`*ðŸ“•Name:* ${cha[0].name}\n${texty}\n*âšœï¸About:* ${
					cha[0].about
				}\n*ðŸ”MAL_ID:* ${cha[0].mal_id}\nðŸŒ *URL:*  ${chara.data.characters.results[0].siteUrl}\n` +
				`\n`;
				await db.set(m.chat, charaoo.name)
await alexa.sendMessage(m.chat,{image:{url:cha[0].images.jpg.image_url},caption:data2},{quoted:m})
}catch(err){
...
alexa.sendMessage(m.chat,{image:{url:srh.data[0].images.jpg.large_image_url},caption:mang},{quoted:m})   
	} catch { 
	return m.reply(`âŒ Couldn't find any results on the term *${q}*`)
	}
...
const wall = new AnimeWallpaper();
    const pages = [1,2,3,4];
        ...
        const wallpaper = await wall
            ...
            .catch(() => null);
			if(!wallpaper) return m.reply(`âŒ Couldn't find any results on the term *${ter}*`)
const i = Math.floor(Math.random() * wallpaper.length);    
            await alexa.sendMessage(m.chat,{image:{url:wallpaper[i].image},caption:`Here you go`},{quoted:m}) 
//alexa.sendMessage(m.chat,{image:{url:wallpaper[i].image},caption:`*Query :* ${q}`})            
break
 ...
 case 'bc': case 'bcgroup': {
  ...
  let txt = `ðŸ”°</ _*Alexa Broadcast*_ >ðŸ”°\n\nðŸ€ *Author:* ${pushname}\n\nðŸ® *Message:* ${bct}`
//const stick=fs.readFileSync(`./trash/right.webp`)
//await alexa.sendMessage(m.chat,{sticker:stick},{quoted:m})
await alexa.sendMessage(i, { video: { url: "https://telegra.ph/file/3c3f94c8463e7f9c29d73.mp4" }, mimetype: 'video/mp4', fileName: `bc.mp4`, caption: `${txt}` })
                    }
                m.reply(`Successfuly Broadcasted in ${anu.length} Groups`)
            }
            break          
            default:
alexa.sendMessage(m.chat,{text:`â— Couldn't find any matching commands. Try again with the commands from the help list`},{quoted:m})
        }
        ...
        const time = moment.tz('Asia/Kolkata').format('DD/MM HH:mm:ss')
        //alexa.sendMessage("12033039020236931@g.us",{text:`*Time:* ${time}\n\n`+`*ERROR:* ${util.format(err)}`})
